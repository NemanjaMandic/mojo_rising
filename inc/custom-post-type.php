...
if( @$contact == 1 ){
	...
	add_action( 'init', 'mojo_contact_custom_post_type' );
	
	add_filter( 'manage_mojo-contact_posts_columns', 'mojo_set_contact_columns' );
...
/* CONTACT CPT */
function mojo_contact_custom_post_type() {
	$labels = array(
		'name' 				=> 'Messages',
...
function mojo_set_contact_columns( $columns ){
	$newColumns = array();
...
function mojo_contact_add_meta_box() {
	add_meta_box( 'contact_email', 'User Email', 'mojo_contact_email_callback', 'mojo-contact', 'side' );
}
function mojo_contact_email_callback( $post ) {
	...
	echo '<input type="email" id="mojo_contact_email_field" name="mojo_contact_email_field" value="' . esc_attr( $value ) . '" size="25" />';
}
